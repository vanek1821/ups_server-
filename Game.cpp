...
bool Game::move(Client* client, int sRow, int sCol, int dRow, int dCol){
    if(checkMove(client, sRow, sCol, dRow, dCol)){
        //if(field[dRow][dRow]->getPiece()!=nullptr)//pokud neprázdný cílový políčko, ulož figurku do seznamu sebranejch figurek
        field[dRow][dCol]->setPiece(field[sRow][sCol]->getPiece());
        field[sRow][sCol]->setPiece(nullptr);
        return true;
    }
    else return false;
}
